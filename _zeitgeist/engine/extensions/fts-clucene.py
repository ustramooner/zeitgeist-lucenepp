...
FILTER_PREFIX_XDG_CATEGORY = "category"
FILTER_PREFIX_APP = "app"
...
class Indexer:
	...
	def __init__ (self, engine):
		...
		self._indexWriter = lucene.IndexWriter(self._directory, self._analyzer, self._maxFieldLength)
		#self._query_parser.set_database (self._index)
		#TODO self._query_parser.add_prefix("name", "N")
		...
		if self._indexWriter.numDocs() == 0:
		...
		try:
			...
			for m in hits:
				log.debug("Deleting event '%s' with docid '%s'" %
				          (event_id, m.docid))
				self._indexWriter.delete_document(m.docid)
		except Exception, e:
	...
	def _index_actor (self, doc, actor):
		...
		if desktop:
			if not desktop.getNoDisplay():
				doc.add(lucene.Field(FILTER_PREFIX_APP, desktop.getName(), lucene.Field.STORE_YES, lucene.Field.INDEX_ANALYZED_NO_NORMS))
				doc.add(lucene.Field(FILTER_PREFIX_APP, desktop.getGenericName(), lucene.Field.STORE_YES, lucene.Field.INDEX_ANALYZED_NO_NORMS))
				doc.add(lucene.Field(FILTER_PREFIX_APP, desktop.getComment(), lucene.Field.STORE_YES, lucene.Field.INDEX_ANALYZED_NO_NORMS))
			
				for cat in desktop.getCategories():
					doc.add(lucene.Field(FILTER_PREFIX_XDG_CATEGORY, cat.lower(), lucene.Field.STORE_YES, lucene.Field.INDEX_ANALYZED_NO_NORMS))
		else:
